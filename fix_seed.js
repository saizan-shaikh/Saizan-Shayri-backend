...
const MONGO_URI = process.env.MONGO_URI || 'mongodb://localhost:27017/shayri_book';

// Define Schema here to avoid import issues
const shayriSchema = new mongoose.Schema({
  text: { type: String, required: true },
  poet: { type: String, required: true },
  category: { type: String, required: true },
  language: { type: String, required: true, default: 'roman' },
}, { timestamps: true });

const Shayri = mongoose.model('Shayri_Fix', shayriSchema, 'shayris'); // Target the 'shayris' collection explicitly
...
async function run() {
  try {
    ...
    await mongoose.connection.collection('shayris').deleteMany({});
    console.log('Cleared.');

    console.log('Inserting 60 shayris...');
    const result = await mongoose.connection.collection('shayris').insertMany(shayris);
    console.log(`Success! Inserted: ${result.insertedCount}`);
    
    // Final verification
    const jaun = await mongoose.connection.collection('shayris').countDocuments({ poet: 'Jaun Elia' });
    console.log(`Jaun Elia Count: ${jaun}`);
